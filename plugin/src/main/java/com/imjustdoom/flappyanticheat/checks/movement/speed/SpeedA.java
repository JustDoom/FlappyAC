...
public class SpeedA extends Check {
    ...
    public void handle(final Packet packet) {
        ...
        if (!data.getFlyingProcessor().isOnGround() && !data.getFlyingProcessor().isLastOnGround()) {
            ...
            double prediction = data.getFlyingProcessor().getLastDeltaXZ() * 0.91f + 0.026;
            double accuracy = (data.getFlyingProcessor().getDeltaXZ() - prediction);
            ...
            if (accuracy > 0.001 && data.getFlyingProcessor().getDeltaXZ() > 0.1) {
                ...
                if (!data.getVelocityProcessor().isTakingVelocity()) {
                    fail("exp=" + prediction + " got=" + data.getFlyingProcessor().getDeltaXZ(), true);
                    ...
                    //testing it can be off by about 0.04, so check if the margin is greater than that.
                        ...
                        data.getFlyingProcessor().getDeltaXZ()) > 0.04) {
                    fail("exp=" + prediction + " got=" + data.getFlyingProcessor().getDeltaXZ() + " vel=" +
